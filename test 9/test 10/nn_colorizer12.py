...
# Initialize network
numL = 4 # number of layers
inLen = 5*5 # length of input vector (should be window squared)
outLen = 3 # length of output vector
biasL = [1, 1, 1, 0] # bias for each layer; 1 = layer has bias, 0 = no bias; last always 0
npL = [(inLen+biasL[0]), 25, 10, outLen] # nodes per layer (includes biases)
actL = [0, 4, 4, 1] # label indicating activation fcn type leading into that layer; 1st is meaningless (see "sigma" fcn)
...
# Train and test nn
epochs = 10 # number of times it trains on the whole training set
