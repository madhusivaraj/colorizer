# This script:
# Initializes a neural network (nn) with specified parameters and random weights.
# Loads a training image that must be called 'image_0001.jpg' and must be in the same directory as the script.
# Trains the nn to map 5x5 windows of pixels of the grayscale version of the training image to the correct RGB values of the window's center pixel.
# Saves the training weights.
# Plots the loss and weight values vs training iterations (needs work).
# Colorizes the grayscale training image using the trained nn and prints the colorized image.
...
# colorize gray image
def colorize(imgBW, window, nodes, weights, biasL, actL):
    imgCL = imageio.imread('image_0001.jpg') # any image with same pixel dimensions as imgBW
...
npL = [(inLen+biasL[0]), 15, outLen] # nodes per layer (includes biases)
actL = [0, 4, 3] # label indicating activation fcn type leading into that layer; 1st is meaningless (see "sigma" fcn)
[nodes, deltas, weights] = nnInitialize(numL, npL) # creates node, delta, and weight lists
lossL = 0 # label indicating loss fcn type (see "loss" and "lossPrime" fcns)

# Train nn
epochs = 1
window = 5
lossList = []
trainingList = ['image_0001.jpg'] # list of names of training images
for i in range(epochs):
    print('Epoch', i)
    for j in range(len(trainingList)):
        print('Training on image', j)
        img = imageio.imread(trainingList[j])
        for L in range(len(img)): # L is pixel row coord
            if np.remainder(L, 5) == 0:
    ...
    #lossList.append(loss(nodes, f, lossL))

# Save weights
outpath = 'out.weights'
with open(outpath, "w") as f:
    f.write(json.dumps(weights)) # to load, weights = json.load(open(outpath))
    print('Weights saved')
#weights = json.load(open('out.weights_lunch1'))

# Test(calculate loss)
#img = imageio.imread('image_0001.jpg') # testing image
#testLoss = 0
#for L in range(len(img)):
    #for W in range(len(img)):
        #[x, f] = ioMaker(img, L, W)
        #nodes = forwardProp(x, nodes, weights, biasL, actL)
        #testLoss += loss(nodes, f, lossL)
#testLoss /= L*W
#print('test loss', testLoss)

# Colorize
img = imageio.imread('image_0001.jpg') # test image
print('Creating grayscale image')
imgBW = colorToGray(img)
print('Colorizing grayscale image')
imgCL = colorize(imgBW, window, nodes, weights, biasL, actL)
fig = plt.figure()
